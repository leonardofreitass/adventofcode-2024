(ns adventofcode-2024.exercises.day-21.part-2
...
(defn numeric-to-directional
  ...
  (first (reduce
          (fn [[acc current] digit]
            [(into acc (path-with-fewest-turns numeric-pad-graph current digit))
             digit])
          [[] \A]
          (vec number))))

(def all-direction-pairs (for [from directions
                               to directions]
                           [from to]))
...
(def pre-computed-directional-costs
  (reduce
   (fn [acc [from to]]
     (assoc acc [from to] (count (path-with-fewest-turns directional-pad-graph from to))))
   {}
   all-direction-pairs))

(defn directional-costs
  [layer commands]
  (first (reduce
          (fn [[a c] n]
            [(+ a (layer [c n])) n])
  ...
  (reduce
   (fn [m [from to]]
     (assoc
      m
      [from to]
      (directional-costs layer (path-with-fewest-turns directional-pad-graph from to))))
   {}
   all-direction-pairs))
...
(defn build-layer-k
  [k]
  (if (= k 0)
    pre-computed-directional-costs
    (build-layer (build-layer-k (dec k)))))
...
(defn run
  ...
  (reduce
   ...
   (map #(->> %
              ...
              (directional-costs (build-layer-k 24))
